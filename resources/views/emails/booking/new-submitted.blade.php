...
@section('content')
    ...
    <p style="margin:0 0 26px; font-family:'Segoe UI',Helvetica,Arial,sans-serif; font-size:15px; line-height:24px; color:#3f3d37;">
        ...
        A customer submitted a @if ($methodLabel)<strong style="color:#112250;">{{ $methodLabel }}</strong> @endif payment reference. Match it against
        that account, then confirm or reject the booking. The slot stays on hold until you do
        @if ($summary['hold_expires_at']) &mdash; until <strong style="color:#112250;">{{ $summary['hold_expires_at'] }}</strong>@endif.
    </p>

    {{-- The number to check, front and centre. --}}
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%; margin:0 0 26px; background-color:#E7E2CE; border-radius:12px;">
        <tr>
            <td style="padding:20px;">
                <p style="margin:0 0 6px; font-family:'Segoe UI',Helvetica,Arial,sans-serif; font-size:11px; line-height:16px; font-weight:700; color:#5c5a52; text-transform:uppercase; letter-spacing:0.7px;">
                    {{ $methodLabel ? $methodLabel.' reference' : 'Payment reference' }}
                </p>
                <p style="margin:0; font-family:'Courier New',Courier,monospace; font-size:22px; line-height:28px; font-weight:700; color:#112250; letter-spacing:1px;">
            ...
            <td align="right" style="padding:20px;">
                ...
                <p class="phea-amount" style="margin:0; font-family:'Segoe UI',Helvetica,Arial,sans-serif; font-size:24px; line-height:28px; font-weight:700; color:#112250; letter-spacing:-0.5px;">
                    {{ $summary['amount'] }}
                </p>
            </td>
        </tr>
